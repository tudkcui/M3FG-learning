...
import args_parser
from utils import get_softmax_action_probs_from_Qs, eval_discounted_curr_minor_reward, eval_discounted_curr_major_reward
...
def plot():
    ...
    for game, variant in zip(games, variants):
        ...
        for num_disc_mf in num_disc_mfs:
            ...
            v_curr_1 = np.vdot(env.mu_0_major,
                               eval_discounted_curr_major_reward(env, mu_disc, action_probs_minor, action_probs_major,
                                                                 gamma=config['gamma'])[0][:, mu_index_initial])
            v_curr_2 = np.vdot(env.mu_0, np.einsum('i,ji', env.mu_0_major,
                                                   eval_discounted_curr_minor_reward(env, mu_disc, action_probs_minor,
                                                                                     action_probs_major,
    ...
    plt.savefig(f'./figures/inf_J_J0_discretization_maxent.pdf', bbox_inches='tight', transparent=True, pad_inches=0.1)
    plt.savefig(f'./figures/inf_J_J0_discretization_maxent.png', bbox_inches='tight', transparent=True, pad_inches=0.1)
    plt.show()
