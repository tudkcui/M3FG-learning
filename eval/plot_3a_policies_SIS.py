...
def plot():
    ...
    for game, variant, num_disc_mf, major_obs in zip(games, variants, num_disc_mfs, major_obss):
        ...
        im = plt.imshow(heatmap.reshape((len(plotted_mfs), env.time_steps)), interpolation='none',
                        ...
                        aspect=env.time_steps,  # norm=mpl.colors.SymLogNorm(linthresh=0.03, linscale=0.03, vmin=0.0, vmax=1.0),
        ...
        plt.title("$\pi^0$ for " + f" $x^0={'H' if major_obs else 'L'}$")
        # plt.grid('on')
        plt.xlabel(r'Time $t$', fontsize=22)
        if i == 2:
            plt.ylabel(r'MF $\mu(I)$', fontsize=22)
        else:
            frame1 = plt.gca()
        ...
        if i == 5:
            cb1 = plt.colorbar(im, fraction=0.046, pad=0.04)
            cb1.set_ticks([0, 0.5, 1], update_ticks=True)
            cb1.set_label(f"$\pi^0(P \mid x^0, \mu)$")
    ...
    for game, variant, num_disc_mf, major_obs in zip(games, variants, num_disc_mfs, major_obss):
        ...
        plt.title("$\pi$ for " + f" $x^0={'H' if major_obs else 'L'}$")
        # plt.grid('on')
        plt.xlabel(r'Time $t$', fontsize=22)
        if i == 2:
            plt.ylabel(r'Infected $\mu(1)$', fontsize=22)
        else:
            frame1 = plt.gca()
            frame1.axes.yaxis.set_ticklabels([])
        ...
        if i == 5:
            cb1 = plt.colorbar(im, fraction=0.046, pad=0.04)
            cb1.set_ticks([0, 0.5, 1], update_ticks=True)
            cb1.set_label(f"$\pi^0(u^0=F \mid x^0, \mu)$, \\\\"
                          f"$\pi(u=P \mid x=S, x^0, \mu)$")
    ...
    """ Finalize plot """
    plt.gcf().set_size_inches(16, 4.25)
    plt.tight_layout(w_pad=0.0)
    plt.savefig(f'./figures/policies_SIS.pdf', bbox_inches='tight', transparent=True, pad_inches=0.1)
    plt.savefig(f'./figures/policies_SIS.png', bbox_inches='tight', transparent=True, pad_inches=0.1)
    plt.show()
