...
def plot():
    ...
    stationary = False
    num_disc_mfs = range(10, 135, 10)

    for game, variant in zip(games, variants):

        itplot = 1000 if game != 'Buffet' else 1000
        ...
        for num_disc_mf in num_disc_mfs:
            try:
                config = args_parser.generate_config_from_kw(game=game, variant=variant, fp_iterations=100 if variant == 'fpi' else 1000 if game != 'Buffet' else 1000,
    ...
    plt.savefig(f'./figures/J_J0_discretization.pdf', bbox_inches='tight', transparent=True, pad_inches=0.1)
    plt.savefig(f'./figures/J_J0_discretization.png', bbox_inches='tight', transparent=True, pad_inches=0.1)
    plt.show()
